...
data_raw = pl.read_csv(datafile, has_header=True).sort(pl.col(['type', 'threads', 'chunk', 'size']))
...
for xtype in exptypes:
    for size in arrsizes:
        ...
        data_time_ch_2 = data_time.filter(pl.col('chunk') == '2').sort(pl.col('threads')).select(pl.col('time'))
        data_time_ch_auto = data_time.filter(pl.col('chunk') == 'auto').sort(pl.col('threads')).select(pl.col('time'))
        # print(data_time_ch_2)
        plot_time.plot(threads, data_time_ch_2, label="Chunk: 2", marker=primarymarkstyle, linestyle=':')
        plot_time.plot(threads, data_time_ch_auto, label="Chunk: auto", marker=secondarymarkstyle, linestyle=':')
        ...
        data_speedup_ch_2 = data_time.filter(pl.col('chunk') == '2').sort(pl.col('threads')).select((t1_ch_2 / pl.col('time')).alias("speedup"))
        data_speedup_ch_auto = data_time.filter(pl.col('chunk') == 'auto').sort(pl.col('threads')).select((t1_ch_auto / pl.col('time')).alias("speedup"))

        plot_sp.plot(threads, data_speedup_ch_2, label="Chunk: 2", marker=primarymarkstyle, linestyle=":")
        plot_sp.plot(threads, data_speedup_ch_auto, label="Chunk: auto", marker=secondarymarkstyle, linestyle=":")
